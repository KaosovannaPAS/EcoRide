...
switch ($method) {
    ...
    case 'PUT':
        ...
        if ($action === 'update_status') {
            ...
            // Admin cancel: trip id passed via GET param
            $tripId = intval($_GET['id'] ?? 0);
            if ($tripId > 0) {
                $fakeData = (object)['trajet_id' => $tripId, 'statut' => 'annule'];
                $res = $controller->updateStatus($fakeData);
            } else {
                $res = ["status" => 400, "message" => "ID trajet manquant."];
            }
        }
        else {
            $res = ["status" => 400, "message" => "Action inconnue."];
